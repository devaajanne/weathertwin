...
public interface WeatherDataRepository extends CrudRepository<WeatherData, Long> {
  ...
  @Query(
      value =
          "SELECT * FROM weatherdatatable WHERE (temp BETWEEN :minTemp AND :maxTemp) AND (lon NOT BETWEEN :minLon AND :maxLon) AND (lat NOT BETWEEN :minLat AND :maxLat) AND (weather_group = :weatherGroup) AND (id != :id)",
      nativeQuery = true)
